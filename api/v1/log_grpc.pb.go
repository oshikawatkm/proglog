...
type LogClient interface {
	...
	ConsumeSream(ctx context.Context, in *ConsumeRequest, opts ...grpc.CallOption) (Log_ConsumeSreamClient, error)
...
func (c *logClient) ConsumeSream(ctx context.Context, in *ConsumeRequest, opts ...grpc.CallOption) (Log_ConsumeSreamClient, error) {
	stream, err := c.cc.NewStream(ctx, &Log_ServiceDesc.Streams[0], "/log.v1.Log/ConsumeSream", opts...)
	if err != nil {
		return nil, err
	}
	x := &logConsumeSreamClient{stream}
...
type Log_ConsumeSreamClient interface {
	Recv() (*ConsumeResponse, error)
	grpc.ClientStream
}

type logConsumeSreamClient struct {
	grpc.ClientStream
}

func (x *logConsumeSreamClient) Recv() (*ConsumeResponse, error) {
...
type LogServer interface {
	...
	ConsumeSream(*ConsumeRequest, Log_ConsumeSreamServer) error
...
func (UnimplementedLogServer) ConsumeSream(*ConsumeRequest, Log_ConsumeSreamServer) error {
	return status.Errorf(codes.Unimplemented, "method ConsumeSream not implemented")
}
...
func _Log_ConsumeSream_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(ConsumeRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(LogServer).ConsumeSream(m, &logConsumeSreamServer{stream})
}

type Log_ConsumeSreamServer interface {
	Send(*ConsumeResponse) error
	grpc.ServerStream
}

type logConsumeSreamServer struct {
	grpc.ServerStream
}

func (x *logConsumeSreamServer) Send(m *ConsumeResponse) error {
...
var Log_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "ConsumeSream",
			Handler:       _Log_ConsumeSream_Handler,
			ServerStreams: true,
		},
		{
